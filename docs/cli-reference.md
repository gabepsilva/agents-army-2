...
| `--prompt-file PATH` | the prompt, read as stripped UTF-8 text from a file |
...
Exactly one of `-p/--prompt`, `--prompt-file PATH`, or `-- PROMPT...` is
required — passing more than one, or none, is an error.
