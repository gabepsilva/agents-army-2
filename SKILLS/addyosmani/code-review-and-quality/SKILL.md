...
Every review evaluates code across these dimensions. When this skill is used from the automated `quality` review stage, tag each finding's `axis` with the matching value in parentheses.

### 1. Correctness (`axis: correctness`)
...
### 2. Readability & Simplicity (`axis: readability`)
...
### 3. Architecture (`axis: architecture`)
...
### 5. Performance (`axis: performance`)
...
| Prefix | Meaning | Author Action | `severity` value in our schema |
|--------|---------|---------------|---------------------------------|
| *(no prefix)* | Required change | Must address before merge | `required` |
| **Critical:** | Blocks merge | Security vulnerability, data loss, broken functionality | `critical` |
| **Nit:** | Minor, optional | Author may ignore — formatting, style preferences | `nit` |
| **Optional:** / **Consider:** | Suggestion | Worth considering but not required | `optional` |
| **FYI** | Informational only | No action needed | *(don't emit as a finding — see note below)* |
